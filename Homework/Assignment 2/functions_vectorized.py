...
def calculate_cost(X, Y, Theta):
    ...
    total_cost = 0
    total_cost = ((X.dot(Theta).flatten() - Y) ** 2).sum() # take the dot product of the X matrix (Nx2) and the Theta matrix (2x1)
...
def linear_regression(X, Y, Theta, alpha, num_iters):
    ...
    for i in range(num_iters):
        for t in range(T):
            total_cost = (((X.dot(Theta).flatten() - Y)) * X[:,t]).sum()
            temps[t][0] = Theta[t][0] - (alpha * (1.0 / N) * total_cost)
        Theta = temps
